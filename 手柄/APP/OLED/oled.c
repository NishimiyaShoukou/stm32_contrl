...
//}
uint8_t Bef[3];//±£´æÇ°Ò»¸öÊý¾ÝµÄ¼¸¸ö²ÎÊý1.ÒªÐ´ÔÚµÚ¼¸Ò³2.0x01ÒªÒÆ¶¯¼¸Î»3.Ð´Ê²Ã´Êý¾Ý
uint8_t Cur[3];//µ±Ç°Ç°Ò»¸öÊý¾Ý1.ÒªÐ´ÔÚµÚ¼¸Ò³2.0x01ÒªÒÆ¶¯¼¸Î»3.Ð´Ê²Ã´Êý¾Ý
...
void Before_State_Update(uint8_t y)//¸ù¾ÝyµÄÖµ£¬Çó³öÇ°Ò»¸öÊý¾ÝµÄÓÐ¹Ø²ÎÊý
{
	Bef[0]=7-y/8;
	Bef[1]=7-y%8;
	Bef[2]=1<<Bef[1];
}
void Current_State_Update(uint8_t y)//¸ù¾ÝYÖµ£¬Çó³öµ±Ç°Êý¾ÝµÄÓÐ¹Ø²ÎÊý
{
	Cur[0]=7-y/8;//Êý¾ÝÐ´ÔÚµÚ¼¸Ò³
	Cur[1]=7-y%8;//0x01ÒªÒÆ¶¯µÄÎ»Êý
	Cur[2]=1<<Cur[1];//ÒªÐ´Ê²Ã´Êý¾Ý
}
...
void OLED_DrawWave(uint8_t x,uint8_t y)
{
	...
	Current_State_Update(y);//¸ù¾ÝYÖµ£¬Çó³öµ±Ç°Êý¾ÝµÄÓÐ¹Ø²ÎÊý
	page_sub=Bef[0]-Cur[0];//µ±Ç°ÖµÓëÇ°Ò»¸öÖµµÄÒ³ÊýÏà±È½Ï
	//È·¶¨µ±Ç°ÁÐ£¬Ã¿Ò»Ò³Ó¦¸ÃÐ´Ê²Ã´Êý¾Ý
	if(page_sub>0)
	{
		page_buff=Bef[0];
